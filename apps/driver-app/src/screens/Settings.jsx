...
function Settings() {
  ...
  // Make sure this matches your Express router setup for drivers!
  const API_URL = "https://ride-hailing-backend-coan.onrender.com/api/drivers";

  const [isEditing, setIsEditing] = useState(false);
  const [editForm, setEditForm] = useState({ name: "", email: "", phone: "" });

  // Fetch logged-in driver data when component mounts
  useEffect(() => {
    const fetchProfile = async () => {
      try {
        // USING driverToken to match your active context
  ...
  const handleSaveProfile = async () => {
    try {
      ...
      // Requires a PUT route on your backend: PUT /api/driver/profile
      ...
      if (response.data.success) {
        const updatedDriver = { ...driver, ...response.data.data };
  ...
  const handleSignOut = () => {
    // Clear driver auth data specifically
          ...
          <div className="max-w-3xl mx-auto w-full space-y-6 pb-10">
            ...
            <div>
              ...
              <p className="text-sm sm:text-base text-brand-teal dark:text-brand-mint mt-1">
                Manage your driver profile, preferences, and account details.
              </p>
            </div>
              ...
              <div className="space-y-5 pt-2">
                ...
                <div className="flex items-center justify-between gap-4">
                  <div className="flex-1">
                    ...
                    <p className="text-xs sm:text-sm text-brand-teal dark:text-brand-mint/80">
                      Receive new ride requests on your device.
                    </p>
                  </div>
                  <button
                ...
                <div className="flex items-center justify-between gap-4">
                  <div className="flex-1">
                    ...
                    <p className="text-xs sm:text-sm text-brand-teal dark:text-brand-mint/80">
                      Get rider details and OTPs via text.
                    </p>
                  </div>
                  <button
